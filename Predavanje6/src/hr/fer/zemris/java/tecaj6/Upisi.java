	...
	private static List<Upis> ucitajUpise(Map<String, Student> studenti,
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(
			...
			while (true) {
				...
				lista.add(new Upis(studenti.get(element[0]), predmeti.get(element[1])));
			}
		}
		return lista;
	}
	...
	private static Map<String, Student> ucitajStudente() throws IOException {
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(
			...
			while (true) {
				...
				String[] element = redak.split("\t");
				mapa.put(element[0], new Student(element[0], element[1],
						element[2]));
			}
		}
		return mapa;
