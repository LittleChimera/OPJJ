...
public abstract class AbstractVector implements IVector {
	...
	public IVector nVectorProduct(IVector other)
		...
		IMatrix calcMatrix = new Matrix(3, 3, new double[][] { { 0, 0, 0 },
				this.toArray(), other.toArray() }, true);
		
		IVector vProduct = newInstance(3);
		for (int i = 0; i < 3; i++) {
			vProduct.set(i, Math.pow(-1, i)
					* calcMatrix.subMatrix(0, i, true).determinant());
		}
	...
	public IMatrix toRowMatrix(boolean liveView) {
		IMatrix viewMatrix = new MatrixVectorView(this, true);
		return (liveView) ? viewMatrix : viewMatrix.copy();
	}
	...
	public IMatrix toColumnMatrix(boolean liveView) {
		IMatrix viewMatrix = new MatrixVectorView(this, false);
		return (liveView) ? viewMatrix : viewMatrix.copy();
	}
